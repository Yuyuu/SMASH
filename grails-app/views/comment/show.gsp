    ...
    <ol class="property-list comment">
        ...
        <g:if test="${commentInstance?.text}">
            <li class="fieldcontain">
                <span id="text-label" class="property-label"><g:message code="comment.text.label"
                                                                        default="Text"/></span>
        ...
        <g:if test="${commentInstance?.author}">
            <li class="fieldcontain">
                ...
                <span class="property-value" aria-labelledby="author-label"><g:link controller="user" action="show"
                                                                                    id="${commentInstance?.author?.id}">${commentInstance?.author?.encodeAsHTML()}</g:link></span>

            </li>
        </g:if>

        <g:if test="${commentInstance?.media}">
            <li class="fieldcontain">
                <span id="media-label" class="property-label"><g:message code="comment.media.label"
                                                                         default="Media"/></span>

                <span class="property-value" aria-labelledby="media-label"><g:link controller="mediaCut" action="show"
                                                                                   id="${commentInstance?.media?.id}">${commentInstance?.media?.encodeAsHTML()}</g:link></span>
